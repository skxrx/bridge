...
import { Bridge, Bridge__factory, Token, Token__factory } from "../typechain";
...
describe("Bridge", function () {
  ...
  const wrongChainId: number = 666;
  const testAddr = "0xbe82fa5E29424659624695F71E4ee7dDa5513c23";

  beforeEach(async function () {
    signers = await ethers.getSigners();
    [validator, user, user2] = [signers[0], signers[1], signers[2]];

    ETH_bridge = await new Bridge__factory(validator).deploy(
      validator.address,
      ETH_Id
    );
    BSC_bridge = await new Bridge__factory(validator).deploy(
      validator.address,
      BSC_Id
    );

    ETH_Token = await new Token__factory(validator).deploy(BSC_bridge.address);
    BSC_Token = await new Token__factory(validator).deploy(ETH_bridge.address);
  ...
  describe("Checking getters", () => {
    it("isTokenSupported = true", async () => {
      expect(
        await ETH_bridge.isTokenSupported(ETH_Token.address, BSC_Id)
      ).to.eq(true);
    });

    it("isTokenSupported = false", async () => {
      expect(
        await ETH_bridge.isTokenSupported(ETH_Token.address, wrongChainId)
      ).to.eq(false);
    });
    ...
    it("nonceStatus = true", async () => {
      await ETH_bridge.connect(user).swap(
        user2.address,
        ETH_Token.address,
        defaultAmount,
        BSC_Id,
  ...
  describe("Checking setters", () => {
    ...
    it("includeToken", async () => {
      await ETH_bridge.includeToken(testAddr, testAddr, wrongChainId);
      expect(await ETH_bridge.isTokenSupported(testAddr, wrongChainId)).to.eq(
        true
      );
    });

    it("includeToken - emit TokenIncluded", async () => {
      await expect(ETH_bridge.includeToken(testAddr, testAddr, wrongChainId))
        .to.emit(ETH_bridge, "TokenIncluded")
        .withArgs(testAddr, testAddr, wrongChainId);
    });
    ...
    it("excludeToken - emit TokenExcluded", async () => {
      await expect(ETH_bridge.excludeToken(ETH_Token.address, BSC_Id))
        .to.emit(ETH_bridge, "TokenExcluded")
        .withArgs(ETH_Token.address, BSC_Id);
    });
  });
  ...
  describe("modifiers", () => {
    it("adminControl", async () => {
      await expect(
        ETH_bridge.connect(user).excludeToken(ETH_Token.address, BSC_Id)
    ...
    describe("swap", () => {
      ...
      it("swap - emit SwapInitilaized", async () => {
        await expect(
          ...
          .to.emit(ETH_bridge, "SwapInitilaized")
          .withArgs(
            user2.address,
            BSC_Token.address,
            BSC_Id,
  ...
  describe("redeem", () => {
    ...
    it("Tokens minted", async () => {
      ...
      await BSC_bridge.connect(user2).redeem(
        user2.address,
        ETH_Token.address,
        defaultAmount,
        1,
